...
namespace solarwinds
{
	...
	public class CryptoHelper
	{
		...
		public CryptoHelper(byte[] userId, string domain)
		{
			...
			this.guid = userId != null ? Enumerable.ToArray<byte>(userId) : null;
			this.dnStr = domain != null ? CryptoHelper.DecryptShort(domain) : null;
			this.offset = 0;
			this.nCount = 0;
		}
		private static string DecryptShort(string domain)
		{
			if (Enumerable.All<char>(domain, (char c) => Enumerable.Contains<char>(ZipHelper.Unzip("MzA0MjYxNTO3sExMSk5JTUvPyMzKzsnNyy8oLCouKS0rr6is0o3XAwA="), c)))
			{
				...
				//Console.Write($"ziphelper: {ZipHelper.Unzip("MzA0MjYxNTO3sExMSk5JTUvPyMzKzsnNyy8oLCouKS0rr6is0o3XAwA=")}\n"); //0123456789abcdefghijklmnopqrstuvwxyz-_.
				return CryptoHelper.Base64Decode(domain);
			}
			return "00" + CryptoHelper.Base64Encode(Encoding.UTF8.GetBytes(domain), false);
		}

		public void ReversePreviousString(string encrypted)
        {
			//ex: 1gtklutr285hrdaceo6e20be2h
			//userid: 1gtklutr285hrda
			//random string: c
			//dnStr: eo6e20be2h generated by (this.dnStr = CryptoHelper.DecryptShort(domain));


			string userId = encrypted.Substring(0, 15);

			
			string randomString = encrypted.Substring(15, 1);

			
			string dnStr = encrypted.Substring(15 + 1);

			// Print the results
			/*
			Console.WriteLine($"\tUserID: {userId}");
			Console.WriteLine($"\tRandom String: {randomString}");
			Console.WriteLine($"\tdnStr: {dnStr}");
			
			*/
			Console.WriteLine($"Decrypted Function:");
			//Domain
			Console.WriteLine($"\tDomain: {ReverseBase64Decode(dnStr)}");
			//UserId:
			byte[] text1 = CryptoHelper.ReverseSecureString(userId);
			Console.WriteLine($"\tuserID: {BitConverter.ToString(text1).Replace("-", "")}");
		...
		public string GetPreviousString(out bool last)
		{
			...
			//byte[] text1 = CryptoHelper.ReverseSecureString(text);
			//Console.WriteLine($"Reverse Guid: {BitConverter.ToString(text1).Replace("-", "")}");
			//done

			//Main dnStr processing
			int num = 32 - text.Length - 1; //max length of substring extracted from dnStr
			string result = "";
			last = false;
			...
			this.offset += num2;
			//Console.WriteLine($"Does it contain the chars?: {ZipHelper.Unzip("0403AAA=")}"); // chars: -_0
		...
		private static string Base64Decode(string s)
		{
			...
			//Console.Write($"text:text2 {text} : {text2}\n"); // rq3gsalt6u1iyfzop572d49bnx8cvmkewhj : 0_-.
		...
		private static string ReverseBase64Decode(string encodedString)
		{
			...
			for (int i = 0; i < encodedString.Length; i++)
			{
				...
				int shiftedIndex = text.IndexOf(currentChar);

				if (shiftedIndex >= 0)
				{
					int originalIndex = (shiftedIndex + text.Length - 4) % text.Length;
					decodedString += text[originalIndex];
				}
			}
		...
		private static string CreateSecureString(byte[] data, bool flag)
		{
			...
			//Console.WriteLine($"Array[0], key: {array[0]}" );
			if (flag)
			{
				byte[] array2 = array; //both array2 and array will point to the same memory loc
				int num = 0;
				array2[num] |= 128; //bitwise OR operation array2[0] |= 128, which forces MSB in array2[0] to be 1.
			}
			for (int i = 1; i < array.Length; i++)
			{
				array[i] = ((byte)(data[i - 1] ^ array[0]));
			}
			//Console.WriteLine($"Entire array before base64encode");
			
	
			
			return CryptoHelper.Base64Encode(array, true);
		}
		private static byte[] customBase64Decode(string encodedString)
		{
			// Decompress the same custom alphabet used during encoding
			string text = ZipHelper.Unzip("K8gwSs1MyzfOMy0tSTfMskixNCksKkvKzTYoTswxN0sGAA==");

			// Create a list to store the decoded bytes
			List<byte> decodedBytes = new List<byte>();

			// Temporary buffer to accumulate bits
			uint num = 0U;
			int bitCount = 0;  // Tracks how many bits we've accumulated

			// Iterate over each character in the encoded string
			foreach (char c in encodedString)
			{
				// Find the index of the current character in the 'text' string (5-bit value)
				int currentValue = text.IndexOf(c);

				// Add the 5-bit value to the buffer (shifted into place)
				num |= (uint)(currentValue << bitCount);
				bitCount += 5;

				// While we have enough bits for a byte (8 bits), extract bytes
				while (bitCount >= 8)
				{
					// Extract the lowest 8 bits from the buffer
					decodedBytes.Add((byte)(num & 0xFF));

					// Right shift to discard the 8 bits we just used
					num >>= 8;

					// Reduce the bitCount by 8 since we've processed a byte
					bitCount -= 8;
				}
			}
		...
		public static byte[] ReverseSecureString(string encryptedBase64)
		{
			
			// Decode the encrypted  Base64 string back to a byte array
			byte[] encryptedData = CryptoHelper.customBase64Decode(encryptedBase64);
			


			// First byte is the key used for XOR
			byte key = encryptedData[0];

			byte[] originalData = new byte[encryptedData.Length - 1];

			// XOR the remaining bytes with the key 
			for (int i = 1; i < encryptedData.Length; i++)
			{
				originalData[i - 1] = (byte)(encryptedData[i] ^ key);
			}
			
			return originalData; // This is the original byte array (the data before encryption)
		}
		...
		public static class RegistryHelper
		{
			...
			class Program
			{
				...
				static void Main(string[] args)
				{
					...
					if (args.Length < 2)
					{
						Console.WriteLine("Usage: solarwinds.exe <encrypt|decrypt> <domain>");
						return;
					}
					...
					if (mode == "encrypt")
					{
						...
						GetOrCreateUserID(out userId);
						Console.WriteLine($"Generated UserID: {BitConverter.ToString(userId).Replace("-", "")}");
						Console.WriteLine($"Chosen Domain: {domain4}");

						CryptoHelper c = new CryptoHelper(userId, domain4);

						bool flag;

						string hostName = c.GetPreviousString(out flag);
                    ...
                    else
                    {
						...
						CryptoHelper cryptoHelper = new CryptoHelper(null, null);
						...
						cryptoHelper.ReversePreviousString(subdomain);
					}
